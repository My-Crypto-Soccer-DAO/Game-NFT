...
const MarketplacePage = () => {
  ...
  const { contract } = useContract(
    "0x23E539FC7174037F1580832A000F0b7b140A2056"
  );
  ...
  const handleBuy = async (tokenId: number, price: number) => {
    try {
      ...
      alert("Erro ao comprar NFT. Tente novamente.");
    }
  };
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="flex flex-col items-center pt-10">
        ...
        {/* Caixa de NFTs */}
        <div className="w-[90%] sm:w-[80%] md:w-[70%] lg:w-[60%] xl:w-[50%] p-4 bg-blue-900 bg-opacity-70 rounded-lg overflow-y-auto max-h-[70vh] shadow-lg">
          {loading && (
            <p className="text-white text-center">Carregando NFTs...</p>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {nfts.length > 0 ? (
              nfts.map((nft, index) => (
                <div
                  key={index}
                  className="bg-white bg-opacity-10 p-4 rounded-lg flex flex-col items-center text-center shadow-md hover:shadow-xl transition-shadow duration-300"
                >
              ...
              <p className="text-white text-center">Nenhum NFT listado.</p>
            )}
          </div>
        </div>
      </div>
